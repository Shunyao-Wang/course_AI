...
int empty_value_sub_bak(node e, int a) //a表示视角是玩家还是电脑，空格评分函数。
{
...
int empty_value_sub(node e, int a) //a表示视角是玩家还是电脑，空格评分函数。
{
	...
	chessbored[e.i][e.j] = temp;
	int b = 1, i, j, k, s = 0, n, fa = black + white - a, b2 = 1;
	/* s总评分，b当前方向评分，a己方颜色，fa对方颜色 */
...
{
	int my, you;
	my = empty_value_sub_bak(e, a);									 //己方评分
	you = empty_value_sub_bak(e, player_color + computer_color - a); //对方评分
	e.value = my + you;
...
{
	int my, you;
	my = empty_value_sub_bak(e, a);
	you = empty_value_sub_bak(e, player_color + computer_color - a);
	e.value = my - you / 2;
	return my - you / 2;
}
